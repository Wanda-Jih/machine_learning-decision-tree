...
def cal_mean_err(DT_record, BT_record, RF_record):
    ...
    return DT_data, BT_data, RF_data

def draw_plot(data_list, DT_data, BT_data, RF_data):
    
    DT_data = np.array(DT_data)
    BT_data = np.array(BT_data)
    RF_data = np.array(RF_data)
    
    plt.errorbar(data_list, DT_data[:, 1], yerr = DT_data[:, 2], label = "Decision Tree")
    plt.errorbar(data_list, DT_data[:, 1], yerr = DT_data[:, 2], label = "Bagging Tree")
    plt.errorbar(data_list, DT_data[:, 1], yerr = DT_data[:, 2], label = "Random Forest")
    
    plt.xlable("Tree depth")
    plt.ylable("Acc")
    plt.legend()
...
if __name__ == '__main__':
    ...
    cal_mean_err(DT_record, BT_record, RF_record)
    
    draw_plot(data_list, DT_data, BT_data, RF_data)
